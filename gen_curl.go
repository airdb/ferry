...
func GenCurlFromRequest(r *http.Request) string {
...
// GenCurlFromParams is less useful than FromRequest because the structure of the
...
func GenCurlFromParams(method string, urlStr string, requestBody string, headers http.Header) string {
...
func getRequestBody(r *http.Request) string {
	...
	readerCopy := io.NopCloser(bytes.NewReader(b))
	readerReplace := io.NopCloser(bytes.NewReader(b))
	r.Body = readerReplace

	data, err := io.ReadAll(readerCopy)
	if err != nil {
